            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if (event != null) {
                    if (event.getAction() == KeyEvent.ACTION_DOWN) {
                        activity.sender.sendCommand(Constants.SPECIAL, Constants.ENTER + ":1");
                    } else if (event.getAction() == KeyEvent.ACTION_UP) {
                        activity.sender.sendCommand(Constants.SPECIAL, Constants.ENTER + ":2");
                    }
                    return true;
                }
                return false;
    ...
    private class TypeInputConnection extends InputConnectionWrapper {
        ...
        public boolean sendKeyEvent(KeyEvent event) {
            ...
            if (event.getAction() == KeyEvent.ACTION_DOWN && event.getKeyCode() == KeyEvent.KEYCODE_DEL) {
                activity.sender.sendCommand(Constants.SPECIAL, Constants.BACK + ":1");
            } else if (event.getAction() == KeyEvent.ACTION_UP && event.getKeyCode() == KeyEvent.KEYCODE_DEL) {
                activity.sender.sendCommand(Constants.SPECIAL, Constants.BACK + ":2");
            }
